    ...
    <div className="w-full max-w-2xl mx-auto space-y-6">
      <div className="text-center space-y-2">
        ...
        <p className="text-muted-foreground">
          We've detected {issues.length} skin concern
          {issues.length !== 1 ? "s" : ""} that need attention
        </p>
      </div>

      <div className="space-y-4">
        {issues.map((issue, index) => {
          // ✅ Auto calculate severity from confidence
          const severity = mapConfidenceToSeverity(issue.confidence);
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 space-y-1">
                    <div className="flex items-center gap-3">
                      <div className={`p-2 rounded-lg ${getSeverityBg(severity)}`}>
                        {severity === "low" ? (
                          <CheckCircle2
                            className={`h-5 w-5 ${getSeverityColor(severity)}`}
                          />
                        ) : (
                          <AlertCircle
                            className={`h-5 w-5 ${getSeverityColor(severity)}`}
